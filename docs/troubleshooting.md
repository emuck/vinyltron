...
the matrix immediately. If `/status` never reports `ready` (for example, Volumio is down),
Vinyltron shows the idle display anyway after 5 minutes rather than staying blank forever.
