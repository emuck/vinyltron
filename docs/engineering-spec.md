...
- **PWM conflict**: GPIO 18 (PWM0) used for matrix OE# — onboard audio must be disabled
  (`dtparam=audio=off` in `/boot/userconfig.txt`, confirmed surviving reboot on Buster).
  On Bookworm (confirmed 2026-06-10), `snd_bcm2835` still loads despite this setting —
  Volumio's custom `volumio.initrd` isn't regenerated by `update-initramfs`, so the
  usual blacklist+rebuild fix doesn't apply. `display.py` checks `/proc/modules` at
  startup and forces `disable_hardware_pulsing = True` if `snd_bcm2835` is loaded,
  trading some flicker for a working display rather than the library's hard `exit(1)`.
  Unpacking `volumio.initrd` (via `unmkinitramfs`) confirmed `snd_bcm2835.ko` isn't
  even bundled in the initramfs, so a cpio unpack/blacklist/repack of the initrd
  wouldn't address the load path either — the module loads from the real rootfs after
  pivot, despite a real-root `/etc/modprobe.d` blacklist+`install .../bin/false`. Not
  worth pursuing further while the runtime auto-fallback works; a real fix needs to
  explain why the real-root modprobe directives aren't honored for this module.
...
- `dtparam=audio=off` kept in `/boot/userconfig.txt`; GPIO18 is reserved for matrix PWM
  timing. On Bookworm this doesn't fully prevent `snd_bcm2835` from loading — see the
  PWM conflict note above; the daemon falls back to `disable_hardware_pulsing = True`
...
requested while that module is loaded.
